...
$(document).ready(function () {
  ...
  $("input:radio[name=group1]").click(function () {
    ...
    $("#ulSourceLang").append(
      '<li role="presentation"><a role="menuitem" tabindex="-1" >Detect Language</a></li>'
    );
    ...
    for (var y in modelList) {
      ...
      if (
        modelListDomain.toLowerCase() === pageDomain.toString().toLowerCase()
      ) {
  ...
  $("#ulSourceLang").on("click", "a", function (e) {
    ...
    $("#dropdownMenuInput")
      .html("")
      .html(sourceLangSelect + '<span class="caret"></span>');
  ...
  $("#sourceTargetLangSwitch").on("click", function (e) {
    ...
    $("#dropdownMenuInput")
      .html("")
      .html(targetLang + '<span class="caret"></span>');
    updateOutputDropdownMenu();
    $("#dropdownMenuOutput")
      .html("")
      .html(sourceLang + '<span class="caret"></span>');
  ...
  $("#home textarea").keyup(function () {
    ...
    if (parseInt($("#home textarea").val().length) <= 0) {
      ...
      if (
        sourceLangSelect.toLowerCase() === "detect language" ||
        sourceLangSelect.toLowerCase() === "choose language"
      ) {
        $("#dropdownMenuInput").html(
          'Choose Language <span class="caret"></span>'
        );
        $("#dropdownMenuOutput").html(
          'Choose Language <span class="caret"></span>'
        );
      }
    }
  ...
  function doneTyping() {
    ...
    if (
      (sourceLangSelect.toLowerCase() === "detect language" ||
        sourceLangSelect.toLowerCase() === "choose language") &&
      parseInt($("#home textarea").val().length) > 0
    ) {
      ...
      $.ajax(restAPICall)
        .done(function (data) {
          ...
          if (langIdentified) {
            ...
            $("#dropdownMenuInput")
              .html("")
              .html(dataLangName + ' <span class="caret"></span>');
          } else {
            ...
            $("#dropdownMenuInput")
              ...
              .html(
                dataLangName +
                  ': not supported for this domain <span class="caret"></span>'
              );
          }
  ...
  function updateOutputDropdownMenu() {
    ...
    $("#dropdownMenuOutput")
      .html("")
      .html('Choose Language <span class="caret"></span>');
    ...
    if ($("#ulTargetLang li").length == 1) {
      $("#dropdownMenuOutput")
        .html("")
        .html($($("#ulTargetLang a")).text() + '<span class="caret"></span>');
    }
  }
  ...
  function getTranslation() {
    ...
    if (
      $("#dropdownMenuInput").text().toLowerCase().indexOf("choose language") <
      0
    ) {
    ...
    if (
      $("#dropdownMenuOutput").text().toLowerCase().indexOf("choose language") <
      0
    ) {
    ...
    if (pageDomain && source && target && textContent && model_id) {
      ...
      var restAPICall = {
        ...
        headers: {
          "X-WDC-PL-OPT-OUT": $(
            "input:radio[name=serRadio]:radio:checked"
          ).val(),
          "X-Watson-Technology-Preview": nmtValue,
        },
        async: true,
  ...
  function getModelId(pageDomain, source, target) {
    ...
    for (var y in modelList) {
      if (modelList[y].hasOwnProperty("domain")) {
        var modelListDomain = modelList[y].domain.toString();
        if (
          modelListDomain.toLowerCase() ===
            pageDomain.toString().toLowerCase() &&
          source === modelList[y].source &&
          target === modelList[y].target
        ) {
    ...
    for (var y2 in modelList) {
      if (modelList[y2].hasOwnProperty("domain")) {
        ...
        if (
          modelListDomain2.toLowerCase() ===
            pageDomain.toString().toLowerCase() &&
          source === getLang(modelList[y2].source) &&
          target === getLang(modelList[y2].target)
        ) {
